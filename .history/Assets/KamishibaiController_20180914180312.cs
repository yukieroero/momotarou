...
public class KamishibaiController : MonoBehaviour {
    ...
    public void action(Person actor, string serif, string position) {
        ...
        balloon.GameObject.GetComponent<RectTransform>().sizeDelta = new Vector2((float)Screen.width, 0f);
        // chatviewにセリフを表示
        GameObject chatview = GameObject.Find("/Canvas/ChatView/Viewport/Content");
        balloon.GameObject.transform.SetParent(chatview.transform); //Assign the newly created Image GameObject as a Child of the Parent Panel.
