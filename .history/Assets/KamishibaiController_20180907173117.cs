...
public class KamishibaiController : MonoBehaviour {
    ...
    public void act(Person actor, string serif, string position) {
        ...
        // RectTransform ActorRectTrans = Actor.AddComponent<RectTransform>(); //Add the Image Component script

        Vector2 anchoredPosition = new Vector2(0, 0);
        if (position == "left") {
            ActorImage.transform.Rotate(new Vector3(0f, 90f, 0f));
            anchoredPosition.x = -canvasWidth / 2 + ActorImage.rectTransform.rect.width;
        ...
        // ActorRectTrans.sizeDelta.Scale(new Vector3(1f, 1f, 1f));
        // Debug.Log(ActorRectTrans);
    }
