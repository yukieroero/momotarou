...
public class KamishibaiController : MonoBehaviour {
    ...
    public Person standActor(Person actor, string position) {
    ...
    public void act(Person actor, string serif, string position) {
        ...
        // 画像を表示させる
        Person Actor = standActor(actor, position);
        // TODO:セリフの表示
        ...
        balloon.transform.GetComponentInChildren<Text>().text = serif;
        balloon.GetComponent<RectTransform>().SetParent(Actor.Actor.transform); //Assign the newly created Image GameObject as a Child of the Parent Panel.
