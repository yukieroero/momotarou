...
namespace Timeline {
    ...
    public class Balloon : AbsCommand {
        ...
        public Balloon (string serif, string position) {
            ...
            ballonLayout.childForceExpandHeight = false;
            ballonLayout.childForceExpandWidth = false;

            GameObject balloonUI = new GameObject();
            balloonUI.name = "ballonUI";
            Image ballonImage = balloonUI.AddComponent<Image>();
            ballonImage.raycastTarget = true;
            ballonImage.fillCenter = true;
            Canvas balloonCanvas = balloonUI.AddComponent<Canvas>();
            balloonCanvas.renderMode = RenderMode.WorldSpace;
            VerticalLayoutGroup ballonUILayout = balloonUI.AddComponent<VerticalLayoutGroup>();
            ContentSizeFitter ballonUIFitter = balloonUI.AddComponent<ContentSizeFitter>();
            ballonUIFitter.horizontalFit = ContentSizeFitter.FitMode.PreferredSize;
            ballonUIFitter.verticalFit = ContentSizeFitter.FitMode.PreferredSize;
            balloonUI.transform.SetParent(balloon.transform);
            balloonUI.SetActive(true);
            ballonUILayout.childControlHeight = true;
            ballonUILayout.childControlWidth = true;
            ballonUILayout.childForceExpandHeight = false;
            ballonUILayout.childForceExpandWidth = false;

            GameObject serifObj = new GameObject();
            serifObj.name = "serif";
            Text serifText = serifObj.AddComponent<Text>();
            serifText.font = FontData.defaultFontData.font;
            ...
            if (position == "right") {
                ballonUILayout.padding = new RectOffset(25, 70, 10, 0);
                balloonLayout.padding.left = 100;
                balloon.GetComponent<RectTransform>().anchoredPosition = new Vector2(-150f, 0f);
                ballonImage.sprite = Resources.Load<Sprite>(jibunBalloon);
            } else if (position == "left") {
                ballonUILayout.padding = new RectOffset(70, 25, 10, 0);
                balloonLayout.padding.right = 100;
                balloon.GetComponent<RectTransform>().anchoredPosition = new Vector2(150f, 0f);
                ballonImage.sprite = Resources.Load<Sprite>(aiteBalloon);
            }
