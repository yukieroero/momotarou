    ...
    public void action(Person actor, string serif, string position) {
        ...
        foreach (Person a in previousActor.Values) {
            if (a != null && a.BalloonList.Count > 0) {
                GameObject latestBalloonObj = a.BalloonList[a.BalloonList.Count - 1].GameObject.transform.Find("balloonUI").gameObject;
                balloonUILayout.padding = new RectOffset(30, 30, 10, 10);
                latestBalloonObj.transform.Find("balloonUI").GetComponent<Image>().sprite = a.getBalloonImg();
                latestBalloonObj..GetComponent<Image>().sprite = a.getBalloonImg();
            }
        }
