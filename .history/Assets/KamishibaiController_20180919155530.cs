...
public class KamishibaiController : MonoBehaviour {
    ...
    public void action(Person actor, string serif, string position) {
        ...
        foreach (Person a in previousActor.Values) {
            if (a.BalloonList.Count > 0) {
                GameObject latestBalloonObj = a.BalloonList[-1].GameObject;
                latestBalloonObj.transform.Find("balloonUI").GetComponent<Image>().sprite = actor.getBalloonImg();
            }
        }
